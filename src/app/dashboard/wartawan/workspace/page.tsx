...
export default async function WartawanWorkspacePage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
    ...
    const [{ data: queue }, { data: mine }, { data: balance }] = await Promise.all([
        // JANGAN ambil `content` di sini: 20 artikel x HTML penuh hanya untuk
        // cuplikan 220 karakter. `excerpt` selalu diisi saat artikel dibuat.
        supabase.from("articles").select("id,title,excerpt,created_at,author:profiles!articles_author_id_fkey(full_name,email),category:categories(name)").eq("status", "in_review").order("created_at", { ascending: true }).limit(20),
