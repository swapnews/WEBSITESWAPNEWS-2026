...
export default async function ChannelPage({ params }: Props) {
    ...
    const [data, headerAd] = await Promise.all([
        getPublicChannelData(slug),
        getActiveAdSlot("global_header_leaderboard"),
    ]);
    if (!data) notFound();
    ...
    return <div className={`channel-page channel-${variant(data.category.slug)} news-app`}>
        <PublicSiteHeader categoryName={data.category.name} tickerText={data.articles.slice(0, 6).map((item) => item.title).join("   •   ")} headerAd={headerAd} />
        <main>
