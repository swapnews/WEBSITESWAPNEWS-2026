...
export default function RedeemForm({ products, balance }: { products: Product[]; balance: number }) {
    ...
    return (
        <form className="member-form" onSubmit={(event) => void submit(event)}>
            <p className="member-balance">Saldo saat ini: <b>{balance} poin</b> (Rp{(balance * 1000).toLocaleString("id-ID")})</p>
            <div className="member-form-grid">
                <label>Tipe redeem<select id="redeem-type" value={type} onChange={(event) => setType(event.target.value as "cash" | "product")}>
                    <option value="cash">Cash (min 100 poin)</option>
                    <option value="product">Tukar produk</option>
                </select></label>
                {type === "cash" && <label>Jumlah poin<input id="redeem-points" name="points" type="number" min={100} max={balance} required /></label>}
            ...
            {type === "cash" && <div className="member-form-grid">
                ...
                <label>Nama pemilik<input id="redeem-owner" name="payout_owner" required maxLength={80} /></label>
            </div>}
